...
for (i in 5:ncol(child.geno)){
  ...
  if (length(chrs) == 1){
    save(curr.unique.win, file = paste0(path,"/", colnames(curr.unique.win)[5], "_", chrs,"_introgression.rda"))
  } else {
    save(curr.unique.win, file = paste0(path,"/", colnames(curr.unique.win)[5],"_introgression.rda"))
  }
}
