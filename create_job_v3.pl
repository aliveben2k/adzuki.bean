#!/usr/bin/perl
#Usage: perl create_job.pl COMMAND_LINE [-cj_help] [-cj_env PATH] [-cj_conda ENV_NAME] [-cj_node INT] [-cj_time TIME] [-cj_ppn INT] [-cj_mem INT] [-cj_qname JOB_NAME] [-cj_queue QUEUE_NAME] [-cj_proj PROJECT_ID] [-cj_module MODULE] [-cj_docker PATH] [-cj_mail EMAIL_ADDRESS] [-cj_qout PATH] [-cj_sn SN] [-cj_exc]
...
chomp(@ARGV);
print "This script is written by Ben Chien. Dec.2024\n";
...
my $qname; my $conda; my @module; my $qout; my $local; my $gpu; my $time;
for (my $i=0;$i<=$#ARGV;$i++){
	if ($ARGV[$i] eq "\-cj_exc"){
		$exc = "-cj_exc ";
		$ARGV[$i] = "";
...
#		splice(@ARGV, $i, 1);
	...
	if ($ARGV[$i] eq "\-cj_time"){
		$time = "-cj_time $ARGV[$i+1] ";
		$ARGV[$i] = "";
		$ARGV[$i+1] = "";
	}
}
...
&pbs_setting("$proj$local@envs$conda$docker$time$gpu$ppn$mem$nodes$ran$mail$qout$query$user_queue$qname@module$exc$c_line");
#print "debug1: $proj$local@envs$conda$docker$ppn$mem$nodes$ran$mail$qout$query$user_queue$qname@module$exc$c_line\n";

sub usage {
	print BOLD "Usage: perl create_job.pl COMMAND_LINE [-cj_help] [-cj_env PATH] [-cj_conda ENV_NAME] [-cj_node INT] [-cj_ppn INT] [-cj_time TIME] [-cj_mem INT] [-cj_qname JOB_NAME] [-cj_docker DOCKER_FILE] [-cj_gpu INT] [-cj_queue QUEUE_NAME] [-cj_proj PROJECT_ID] [-cj_module MODULE] [-cj_qout PATH] [-cj_sn SN] [-cj_mail EMAIL_ADDRESS] [-cj_exc]\n", RESET;
...
sub help {
	...
	print "-cj_time\tSet runtime. Eg. 168:00:00\n";
	...
	print "-cj_gpu\tA GPU core(s) to use. Only works for NARO's server.\n";
