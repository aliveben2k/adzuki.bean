...
for (my $i=0; $i<=$#ARGV; $i++){
	...
	if ($ARGV[$i] eq "\-cb" || $ARGV[$i] eq "\--clue-bins"){ #set boundary of the year for CLUES plotting
        ...
        foreach my $j (0..$#cbins){
			unless (-e $cbins[$j]){
				die "-cb: Cannot find the bin file: $cbins[$j] for CLUES.\n";
			}
    ...
    if ($ARGV[$i] eq "\-cf"){
    	$cf = $ARGV[$i+1];
    	unless ($cf == "a" || $cf == "b"){ #CLUES: output format for Sample branch lengths function
    		die "-cf: only support format a and b now.\n";
    	}
    }
...
if (@cbins){
	if (@cbins){
		if (scalar(@cbins) == 1){
			foreach my $i (1..$#cbins){
				push(@cbins, $cbins[0]);
			}
		}
		elsif (scalar(@bps) != scalar(@cbins)){
			die "-cbin: The number of defined color should be 1 or the same with -bp.\n";
		}
	}
}
...
foreach (@vcfs){
    ...
    unless (-e "$o_path\/inputs\/$ran\_input_chr$num.haps" &&  -e "$o_path\/inputs\/$ran\_input_chr$num.sample" && $ow == 0){
        	...
        	$qout .= "perl vcf2anc_vcf.pl -vcf $_ -aid $al -keep -bi -nm -sf $anc_hap\-rchr $o_path\/rename_chr.list -o $o_path\/inputs\/$ran\_input_chr$num\\n";
    }
    #prepare input files
    if (@masks){
...
foreach my $z (1..$rc){
    ...
    unless (@popis){ #without population of interests
		...
		foreach my $i (0..$#clues_chr){
			...
			if ($clues == 1){
				unless (-e "$o_path\/CLUES\/ALL\/$ran\_$z\_CLUES_chr$clues_chr[$i]\_$clues_fbp[$i]\-$clues_lbp[$i].timeb" && $ow == 0){
		...
		foreach my $l (0..$#popis){
			...
			foreach my $i (0..$#clues_chr){
				...
				if ($clues == 1){
					...
					unless (-e "$o_path_clues[$l]\/$ran\_$z\_CLUES$popi_out_name\_chr$clues_chr[$i]\_$clues_fbp[$i]\-$clues_lbp[$i].timeb" && $ow == 0){
