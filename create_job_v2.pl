...
foreach (@server){
	if ($_ =~ /140.112.2.71/){
		$serv = 2;
		print BOLD "h71 server is detected. Set to the server\'s PBS job setting.\n\n", RESET;
	}
	if ($_ =~ /140.112.2.74/){
		$serv = 2;
		print BOLD "h71 server is detected. Set to the server\'s PBS job setting.\n\n", RESET;
	...
	if ($_ =~ /140.112.2.73/){
		$serv = 2;
		print BOLD "gc3 server is detected. Set to the server\'s PBS job setting.\n\n", RESET;
	}
    if ($_ =~ /140.112.2.81/){
		$serv = 2;
		print BOLD "h81 server is detected. Set to the server\'s PBS job setting.\n\n", RESET;
	}
    if ($_ =~ /172.28.111/){
		$serv = 3;
		print BOLD "Taiwania 3 server is detected. Set to the server\'s Slurm job setting.\n\n", RESET;
	}
}
if ($serv == -1){
