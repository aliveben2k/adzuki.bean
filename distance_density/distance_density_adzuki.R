...
for(i in 1:length(uni.group)){
  for (k in i:length(uni.group)){
    ...
    if (uni.group[i] == uni.group[k]){
      p1.vs.p2.dist[lower.tri(p1.vs.p2.dist)] <-NA
      p1.vs.p2.dist[p1.vs.p2.dist == 0] <- NA
      
    }
    p1.vs.p2.dist <- as.vector(p1.vs.p2.dist)
    p1.vs.p2.dist <- p1.vs.p2.dist[!is.na(p1.vs.p2.dist)]
...
#write.table(final.table, file = "/Users/ben/Library/CloudStorage/GoogleDrive-aliveben@gmail.com/Other\ computers/Ben_Home_PC/cloud_files/Lee\ lab/Vigna\ angularis/new_gatk4/final_no_out/20211222_0.7/m_dist_density_paper.txt", quote = F, sep = "\t", col.names = T, row.names = T)
...
out.plot <- ggplot(data.frame(final.table), aes(x = as.numeric(x), y = as.numeric(y), group = group)) +
  ...
  scale_x_continuous(breaks = seq(0.000, 0.025, by = 0.010), name = "Pairwise distance") +
  ylab("Density") +
  theme_minimal() +
...
name_a <- "/Users/ben/Library/CloudStorage/GoogleDrive-aliveben@gmail.com/Other\ computers/Ben_Home_PC/cloud_files/Lee\ lab/Vigna\ angularis/new_gatk4/final_no_out/20211222_0.7/distance_density_north.tiff"
